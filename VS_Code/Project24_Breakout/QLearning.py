...
class QNet(nn.Module):
    def __init__(self, hidden1=512, hidden2=64, out_dim=3):
        ...
        self.Conv_layers = nn.Sequential(
            nn.MaxPool2d(2, 2),  # 2x76x46
            nn.Conv2d(2, 1, kernel_size=5, padding=2),  # 1x76x46
            nn.MaxPool2d(2, 2),  # 1x38x23
            nn.Flatten()
        )
        self.fc_layers = nn.Sequential(
            nn.Linear(38*23, hidden1),
            nn.ReLU(),
...
def main():
    ...
    torch.save(Q, "project24_Breakout/QLearning_model.pth")
